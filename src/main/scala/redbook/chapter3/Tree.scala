...
object Tree {
  ...
  def size[A](t: Tree[A]): Int = fold(t)(_ => 1)((lhs, rhs) => 1 + lhs + rhs)

  def maximum[A](t: Tree[A])(max: (A, A) => A): A = fold(t)(identity)(max)

  def depth[A](t: Tree[A]): Int = fold(t)(_ => 1)((lhs, rhs) => 1 + Math.max(lhs, rhs))
